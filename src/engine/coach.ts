...
export function buildCoachSystemPrompt(f: CoachFacts, profileSummary: string, extras?: CoachPromptExtras): string {
  ...
  const lines = [
    ...
    '- Metric units. Keep answers short (under 120 words) unless asked for detail. No emojis.',
    '',
  ...
  if (baseline) lines.push('', 'STARTING POINT (from onboarding)', baseline)
  const reports = (extras?.reports ?? []).map((r) => r.trim()).filter(Boolean)
  if (reports.length) {
    lines.push('', 'HEALTH REPORTS (shared by the user; values and ranges exactly as printed on each report)', ...reports.map((r) => `- ${r}`), REPORTS_CONTEXT_RULE)
  }
  return lines.join('\n')
}
