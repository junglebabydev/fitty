...
import { addSet, deleteSet, exerciseHistory, lastSetsForExercise, previousSetsForExercise, updateVoiceCommand } from '../../db/repositories'
import { TIMED_IDS, type ProgressionAction, type ProgressionResult } from '../../engine'
...
export interface ExerciseCardProps {
  ...
  painNext: boolean
  onLogged(set: ExerciseSet & { restSec: number; pr: LivePR | null }): void
  onSubstitute(): void
  onPain(): void
}
...
const NO_LOAD_EQUIPMENT = ['bodyweight', 'bike', 'treadmill', 'elliptical', 'pool', 'none']
...
export function ExerciseCard({ session, index, planned, exercise, sets, library, stopped, painNext, onLogged, onSubstitute, onPain }: ExerciseCardProps) {
  ...
  const log = () => {
    if (!canLog) return
    const row: Omit<ExerciseSet, 'id'> = {
      sessionId: session.id,
      exerciseId: exercise.id,
      setIndex: sets.length + 1,
      reps: timed ? null : reps,
      loadKg: loadable ? load : null,
      rir: timed ? null : rir,
      rpe: null,
      durationSec: timed ? duration : null,
      painFlag: painNext,
      loggedAt: nowIso(),
    }
    ...
    const id = addSet(row)
    if (pendingVoice.current) {
      try { updateVoiceCommand(pendingVoice.current, { status: 'applied' }) } catch { /* ignore */ }
      pendingVoice.current = 0
    }
    setHint(null)
    setPoppedId(id)
    onLogged({ ...row, id, restSec: planned.restSec, pr })
  }
